...
learningRates = 0.05
numRepetitions = 4
hiddenLayerSizes = [5, 25, 50, 75, 100, 150, 200, 500]
...
mean_train_accuracy = np.zeros(len(hiddenLayerSizes))
std_train_accuracy = np.zeros(len(hiddenLayerSizes))
mean_eval_accuracy = np.zeros(len(hiddenLayerSizes))
std_eval_accuracy = np.zeros(len(hiddenLayerSizes))
mean_test_accuracy = np.zeros(len(hiddenLayerSizes))
std_test_accuracy = np.zeros(len(hiddenLayerSizes))

for k in range(len(hiddenLayerSizes)):
    ...
    for i in range(numRepetitions):
        print("Iteration %d..." % i)
        net = nn.MLPClassifier(hidden_layer_sizes=hiddenLayerSizes[k], activation='logistic', solver='sgd', alpha=0.0, 
...
ax.errorbar(hiddenLayerSizes, mean_eval_accuracy, yerr=std_eval_accuracy, label = 'validation accuracy')
ax.errorbar(hiddenLayerSizes, mean_test_accuracy, yerr=std_test_accuracy, label = 'test accuracy')
...
print("HiddenLayers tested: %s" % hiddenLayerSizes)
